...
export class HuiAreaCard extends LitElement implements LovelaceCard {
  ...
  public getCardSize(): number {
    const firstFeaturePosition =
      this._config && this._firstFeaturePosition(this._config);
    const displayType = this._config?.display_type || "picture";
    const featuresCount = this._config?.features?.length || 0;
    return (
      1 +
      (displayType === "compact" ? (this._config?.vertical ? 1 : 0) : 2) +
      (firstFeaturePosition === "inline" ? featuresCount - 1 : featuresCount)
    );
  }
  ...
  public getGridOptions(): LovelaceGridOptions {
    ...
    let min_columns = 6;
    let rows = 1;
    const firstFeaturePosition = this._config
      ? this._firstFeaturePosition(this._config)
      : "bottom";
    const featuresCount = this._config?.features?.length || 0;
    if (featuresCount) {
      if (firstFeaturePosition === "inline") {
        min_columns = 12;
        rows += featuresCount - 1;
      } else {
        rows += featuresCount;
      }
    }
    ...
    if (displayType !== "compact") {
      if (firstFeaturePosition === "inline" && featuresCount > 0) {
        rows += 3;
      } else {
        rows += 2;
      }
  ...
  private _firstFeaturePosition = memoizeOne((config: AreaCardConfig) => {
    if (config.vertical) {
      return "bottom";
    }
  ...
  private _displayedBottomFeatures = memoizeOne((config: AreaCardConfig) => {
    const features = config.features || [];
    const firstFeaturePosition = this._firstFeaturePosition(config);
  ...
  protected render() {
    ...
    const firstFeaturePosition = this._firstFeaturePosition(this._config);
    ...
    return html`
      <ha-card style=${styleMap(style)}>
        ...
        <ha-tile-container
          .firstFeaturePosition=${firstFeaturePosition}
        ...
        >
          ...
          ></ha-tile-info>
          ${
            features.length > 0 && firstFeaturePosition === "inline"
              ? html`
                  <hui-card-features
                    slot="inline-feature"
                    .hass=${this.hass}
          ...
          ${
            (features.length > 0 && firstFeaturePosition === "bottom") ||
            (features.length > 1 && firstFeaturePosition === "inline")
              ? html`
                  <hui-card-features
                    slot="bottom-features"
                    .hass=${this.hass}
                    .context=${this._featureContext}
                    .color=${this._config.color}
                    .features=${this._displayedBottomFeatures(this._config)}
                    .position=${"bottom"}
                  ></hui-card-features>
                `
              : nothing
